...
#include "Controller/Enemy_AIController.h"
...
ATP_ThirdPersonCharacter::ATP_ThirdPersonCharacter()
{
	...
	DamageSphereComponent = CreateDefaultSubobject<USphereComponent>(TEXT("DamageSphereComponent"));
	DamageSphereComponent->SetupAttachment(GetRootComponent());
	DamageSphereComponent->OnComponentBeginOverlap.AddDynamic(this,&ThisClass::HitOverlap);
	DamageSphereComponent->OnComponentEndOverlap.AddDynamic(this,&ThisClass::EndOverlap);
	
	HealthComponent = CreateDefaultSubobject<UHealthComponent>(TEXT("HealthComponent"));
	ScoreValue = 10;
	DamageFrequency = 1.0f;
	DamageAmount = 10.0f;
...
void ATP_ThirdPersonCharacter::Tick(float DeltaSeconds)
{
	...
	if (AIController)
	{
		AIController->MoveToActor(GetWorld()->GetFirstPlayerController());
	}
	CheckIsDead();
	
}

void ATP_ThirdPersonCharacter::CheckIsDead()
{
...
void ATP_ThirdPersonCharacter::HitOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	DamageTarget = Cast<AFPS_HomeWorkCharacter>(OtherActor);
	if (DamageTarget)
	{
		GetWorld()->GetTimerManager().SetTimer(DamageTimerHandle ,this, &ThisClass::Damage,DamageFrequency,true,0.0f);
		
	}
}

void ATP_ThirdPersonCharacter::EndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	AFPS_HomeWorkCharacter* PlayerCharacter = Cast<AFPS_HomeWorkCharacter>(OtherActor);
	if (PlayerCharacter)
	{
...
void ATP_ThirdPersonCharacter::Damage()
{
	if (DamageTarget)
	{
		FDamageEvent DamageEvent;
		DamageTarget->TakeDamage(DamageAmount,DamageEvent,nullptr,GetOwner());
	}
}
